...
import { faCircleDot, faCheck, faCaretDown } from "@fortawesome/free-solid-svg-icons"
  ...
  return (
    <div className="flex justify-between w-full p-4 border rounded-t-md border-gray-200 bg-gray-50">
      <div className="flex items-center gap-x-3">
        <button
          className={`flex items-center gap-x-1 ${searchFilters.open ? 'text-blue-500' : ''}`}
          onClick={() => setSearchParam('open', true)}
        >
          <FontAwesomeIcon icon={faCircleDot} />
          <p className={`font-semibold text-gray-900 ${searchFilters.open ? 'text-blue-500' : ''}`}>Open</p>
        </button>
        ...
        >
          ...
          <p className={`font-semibold text-gray-900 ${!searchFilters.open ? 'text-blue-500' : ''}`}>Closed</p>
        </button>
      </div>
